...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune a Gemma model with LoRA.")
    
    # Data and Model Configuration
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size for training.")
    parser.add_argument("--dataset_name", type=str, default="mtnt/en-fr", help="Name of the dataset from Hugging Face.")
    parser.add_argument("--max_target_length", type=int, default=256, help="Maximum sequence length for target outputs.")
    parser.add_argument("--model_name", type=str, default="google/gemma/flax/2b", help="Base model name from Kaggle Hub.")
    parser.add_argument("--model_version", type=str, default="2b", help="A string identifier for the model version.")

    # LoRA Parameters
    parser.add_argument("--rank", type=int, default=16, help="The rank of the LoRA matrices.")
    parser.add_argument("--alpha", type=float, default=2.0, help="The scaling factor for LoRA.")
    parser.add_argument("--lora_modules", type=str, default=".*q_einsum|.*kv_einsum|.*gate_proj|.*down_proj|.*up_proj", help="Regex to specify layers for LoRA.")

    # Training Hyperparameters
    parser.add_argument("--max_steps", type=int, default=500, help="Total number of training steps.")
    parser.add_argument("--num_epochs", type=int, default=3, help="Number of training epochs.")
    parser.add_argument("--learning_rate", type=float, default=1e-3, help="Learning rate for the optimizer.")
    parser.add_argument("--eval_every_n_steps", type=int, default=20, help="Evaluation frequency.")

    # File Paths
    parser.add_argument("--intermediate_ckpt_dir", type=str, default="/home/shivajid/intermediate_ckpt/", help="Directory for intermediate checkpoints.")
    parser.add_argument("--ckpt_dir", type=str, default="/home/shivajid/ckpts/", help="Root directory for final checkpoints.")
    parser.add_argument("--profiling_dir", type=str, default="/home/shivajid/profiling/", help="Directory for profiling traces.")
    parser.add_argument("--servable_ckpt_dir", type=str, default="/home/shivajid/safetensors_ckpt/v3/", help="Output directory for the final .safetensors model.")
...
kaggle_ckpt_path = kagglehub.model_download(args.model_name)


params = params_lib.load_and_format_params(os.path.join(kaggle_ckpt_path, args.model_version))

gemma = gemma_lib.Transformer.from_params(params, version=args.model_version)
...
profiler_option = profiler.ProfilerOptions(log_dir=args.profiling_dir, skip_first_n_steps=2, profiler_steps=25)
...
lora_trainer = peft_trainer.PeftTrainer(
    lora_gemma, optax.adamw(args.learning_rate), training_config
...
trained_ckpt_path = os.path.join(args.ckpt_dir, str(args.max_steps), "model_params")
...
with jax.default_device(jax.devices("cpu")[0]):
    hf_weights = model_state_to_hf_weights(lora_state,rank=args.rank, alpha=args.alpha)
